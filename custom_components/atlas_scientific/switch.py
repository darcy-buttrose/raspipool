...
class AtlasSwitch(Entity):
    ...
    def dose_by_volume(self, volume):
        _LOGGER.debug("dose_by_volume for %s with %s " % ( self.name, volume ))
        try:
            cmd = f"D,{volume}"
            r = self._read(cmd)
            _LOGGER.debug("dose_by_volume for %s/%s => %s" % ( self.name, cmd, r ) )
    ...
    def i2c_read(self, num_of_bytes=31):
        ...
        _LOGGER.debug("I2C read response: " + str(response))
        if response[0] == 1:             # if the response isn't an error
            ...
            char_list = map(lambda x: chr(x & ~0x80), list(response[1:]))
            _LOGGER.error("I2C read error(" + str(response[0]) + ") => " + ''.join(char_list))
            return ''

    def _read(self,command="D,?",terminator="\r*OK\r"):
        line = ""
        if self.io_mode == 0:
    ...
    def update(self):
        ...
        try:
            r = self._read()
            _LOGGER.debug("update %s/D => '%s'" % ( self.name, r ) )
            r = self._read("PV,?")
            _LOGGER.debug("update %s/PV => '%s'" % ( self.name, r ) )
